module.exports = {
    ...
    UPDATE_EXPIRY_LAB_CLOUDSLICE:`UPDATE cloudsliceorgassignment  SET enddate = Greatest(NOW() , enddate)  + ($1 || ' days ')::interval, status = CASE WHEN status = 'expired' THEN 'active' ELSE status END WHERE purchased_id=$2`,
    UPDATE_EXPIRY_LAB_SINGLEVMPROXMOX:`UPDATE singlevmproxmoxorgassignment  SET enddate = Greatest(NOW() , enddate)  + ($1 || ' days ')::interval, status = CASE WHEN status = 'expired' THEN 'active' ELSE status END WHERE purchased_id=$2`,
    UPDATE_EXPIRY_LAB_SINGLEVMDATACENTER:`UPDATE singlevmdatacenterorgassignment  SET enddate = Greatest(NOW() , enddate)  + ($1 || ' days ')::interval, status = CASE WHEN status = 'expired' THEN 'active' ELSE status END WHERE purchased_id=$2`,
    UPDATE_EXPIRY_LAB_VMCLUSTERDATACENTER:`UPDATE vmclusterdatacenterorgassignment  SET enddate = Greatest(NOW() , enddate)  + ($1 || ' days ')::interval, status = CASE WHEN status = 'expired' THEN 'active' ELSE status END WHERE purchased_id=$2`,
